...
function generatePackageNameWithMessageFactory(opts) {
  ...
  return function generatePackageNameWithMessage() {
    ...
    const fileStrWithTitle = fileStr.replace(/<%= TITLE %>/, projectName);
    console.log(fileStrWithTitle, 'fileStrWithTitle');

    return fs.writeFileSync(file, fileStrWithTitle, 'utf8');
  }
}
